...
if __name__ == "__main__":
    ...
    for day in ["06", "08"]:
        ...
        for (source, sink), groups in source_to_sink_groups[day].items():
            ...
            for size, trajectory in meta_trajectories[day][(source, sink)].items():
                ax.plot(
                    trajectory[:, 1],
                    trajectory[:, 2],
                    linewidth=2,
            ...
            plt.tight_layout()
            plt.savefig(
                f"../data/figures/02_05_meta_trajectories_wrt_size/{day}/02_04_{source}_{sink}_meta_trajectory.png"
            )
            plt.close()
